...
export class ModelsService {
  ...
  /* 获取首页模型预设 */
  async getHomePresets() {
    try {
      // 先从配置中获取自定义预设
      const customPresetsConfig = await this.configEntity.findOne({
        where: { configKey: 'homeModelPresets' }
      });

      if (customPresetsConfig && customPresetsConfig.configVal) {
        try {
          const customPresets = JSON.parse(customPresetsConfig.configVal);
          return customPresets;
        } catch (parseError) {
          Logger.warn(`解析自定义预设失败，使用默认预设: ${parseError.message}`, 'ModelsService');
        }
      }

      // 如果没有自定义预设或解析失败，则获取启用的模型列表，按排序排列
      const enabledModels = await this.modelsEntity.find({
        where: { status: true },
      ...
      // 转换为预设格式
      const presets = enabledModels.map(model => ({
        id: model.id,
        title: model.modelName,
        description: model.modelDescription || `${model.modelName} - 智能对话模型`,
        icon: 'ri:robot-line',
        iconColor: this.getModelColorClass(model.keyType),
        model: model.model,
        ...
        deductDeepThink: model.deductDeepThink,
        isMcpTool: model.isMcpTool,
        systemPrompt: model.systemPrompt,
        systemPromptType: model.systemPromptType,
        prompt: `你好，我是${model.modelName}，很高兴为您服务！`
      }));

      return presets;
    } catch (error) {
      Logger.error(`获取首页模型预设失败: ${error.message}`, 'ModelsService');
      throw new HttpException('获取首页模型预设失败', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  /* 设置首页模型预设 */
  async setHomePresets(params: { presets: any[] }) {
    try {
      const { presets } = params;
      
      // 验证预设数据
      if (!Array.isArray(presets)) {
        throw new HttpException('预设数据格式错误', HttpStatus.BAD_REQUEST);
      }
      ...
      if (existingConfig) {
        ...
        await this.configEntity.update(
          { configKey: 'homeModelPresets' },
          {
            configVal: presetsJson,
            status: 1,
            public: 1 // 设为公开配置，前端可以访问
          }
        );
      } else {
        // 创建新配置
        await this.configEntity.save({
          configKey: 'homeModelPresets',
          configVal: presetsJson,
          status: 1,
          public: 1, // 设为公开配置，前端可以访问
      ...
      return { success: true, message: '首页模型预设设置成功' };
    } catch (error) {
      Logger.error(`设置首页模型预设失败: ${error.message}`, 'ModelsService');
      throw new HttpException('设置首页模型预设失败', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }

  /* 根据模型类型获取颜色类 */
  private getModelColorClass(keyType: number): string {
    const colorMap = {
      1: 'text-blue-500',    // 普通对话
      2: 'text-purple-500',  // 绘画
      3: 'text-green-500',   // 高级对话
      4: 'text-orange-500',  // 其他类型
      5: 'text-red-500',     // 其他类型
    };
    return colorMap[keyType] || 'text-gray-500';
  }
}
