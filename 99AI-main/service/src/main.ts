  ...
  try {
    redis = new Redis({
      ...
      retryStrategy: (times) => {
        ...
        Logger.warn(`Redis 连接重试，第 ${times} 次，延迟 ${delay}ms`, 'Bootstrap');
        return delay;
      },
    ...
    // 添加错误处理
    redis.on('error', (err) => {
      Logger.error(`Redis 连接错误: ${err.message}`, 'Bootstrap');
    });

    redis.on('connect', () => {
      Logger.log('Redis 连接成功', 'Bootstrap');
    });
    ...
    try {
      ...
      if (!existingSecret) {
        ...
        Logger.log('Generating and setting new JWT_SECRET', 'Bootstrap');
        await redis.set('JWT_SECRET', jwtSecret);
      }
    } catch (redisError) {
      Logger.warn(`Redis 操作失败，但继续启动: ${redisError.message}`, 'Bootstrap');
    }
  } catch (redisConnectionError) {
    Logger.error(
      `Redis 连接失败，但继续启动应用: ${redisConnectionError.message}`,
      'Bootstrap',
    );
    Logger.warn('应用将在没有 Redis 的情况下启动，某些功能可能不可用', 'Bootstrap');
  }
