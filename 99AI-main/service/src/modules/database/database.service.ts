...
export class DatabaseService implements OnModuleInit {
  ...
  async createDefaultUser(userInfo: UserInfo) {
    try {
      ...
      Logger.log(
        `初始化创建${role}用户成功、用户名为[${username}]、初始密码为[${username === 'super' ? 'super' : '123456'
        }] ==============> 请注意查阅`,
        'DatabaseService',
      );
    } catch (error) {
...
`;
      ...
      const res = await this.connection.query(
        `INSERT INTO config (configKey, configVal, public, encrypt) VALUES ${defaultConfig
          .map(
            d =>
              `('${d.configKey}', '${d.configVal.replace(/'/g, "\\'")}', '${d.public}', '${d.encrypt
              }')`,
          )
          .join(', ')}`,
      );
